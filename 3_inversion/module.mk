...
OSSE_CASES = random1-LG-TRUEBIAS-FIXEDAO-FIXEDWO5 \
	...
	random1-LG-TRUEBIAS-NOBIAS-RHO0-FIXEDHYPER \
	...
	random1-LN-TRUEBIAS-NOBIAS-RHO0-FIXEDHYPER
...
$(SENSITIVITIES): $(3_INVERSION_SOURCE)/sensitivities.R $(CONTROL_MOLE_FRACTION) $(INVERSION_BASE_PARTIAL) $(UTILS_PARTIAL)
	Rscript $< \
		--control $(CONTROL_MOLE_FRACTION) \
		--matched-runs-directory $(GEOS_CHEM_MATCHED_RUNS) \
		--output $@

$(CONTROL_MOLE_FRACTION): $(3_INVERSION_SOURCE)/control-mole-fraction.R $(INVERSION_BASE_PARTIAL)
	Rscript $< \
		--matched-runs-directory $(GEOS_CHEM_MATCHED_RUNS) \
		--obspack-directory $(OBSPACK_DATA) \
		--start-date $(INVERSION_START_DATE) \
...
$(PERTURBATIONS): $(3_INVERSION_SOURCE)/perturbations.R $(CONTROL_EMISSIONS) $(INVERSION_BASE_PARTIAL) $(UTILS_PARTIAL)
	Rscript $< \
		--control $(CONTROL_EMISSIONS) \
		--matched-runs-directory $(GEOS_CHEM_MATCHED_RUNS) \
		--output $@

$(CONTROL_EMISSIONS): $(3_INVERSION_SOURCE)/control-emissions.R $(INVERSION_BASE_PARTIAL)
	Rscript $< \
		--matched-runs-directory $(GEOS_CHEM_MATCHED_RUNS) \
		--start-date $(INVERSION_START_DATE) \
		--end-date $(INVERSION_END_DATE) \
