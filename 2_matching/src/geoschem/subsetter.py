...
def match_to_nearest(haystack, needles):
    ...
    other_closer = np.abs(
        haystack[other_indices] - needles
    ) < np.abs(
...
def process_history_file(
    ...
    with xarray.open_dataset(filename)[[
        ...
        assert(is_same_grid(
            GEOS_CHEM_GRID['latitude'],
            ds['model_latitude'].values
        ))
        assert(is_same_grid(
            GEOS_CHEM_GRID['longitude'],
            ds['model_longitude'].values
        ))
        observation_times = observation_ds['time'][start_index:end_index] \
            .values
        time_indices = xarray.DataArray(
            match_to_nearest(ds['time'].values, observation_times),
            dims='sounding_id'
        )
        observation_lons = observation_ds[
            'longitude'
        ][
        ...
        observation_lats = observation_ds['latitude'][start_index:end_index] \
            .values
        ...
        sounding_id = observation_ds['sounding_id'][
            start_index:end_index
        ]
...
class GEOSChemSubsetter(object):
    def __init__(
        ...
        self.observation_days = self.observation_ds['time'] \
            .values \
            .astype('datetime64[D]')
    ...
    def _match_history_files(
        ...
        history_files = np.array(glob.glob(os.path.join(
            run_directory,
            'output',
            pattern
        )))
