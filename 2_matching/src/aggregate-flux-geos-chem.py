...
parser.add_argument(
    ...
    type=str,
    help=(
        'Base directory of runs (typically 1_transport/intermediates/'
        'GEOS_Chem/runs)'
    ),
    required=True
)
...
def aggregate_run(run):
    ...
    emission_files = sorted(glob.glob(os.path.join(
        os.path.join(run['full_path'], 'output'),
    ...
    output['duration'] = 60 * 60 * grouped_by_ds \
        .count()['flux_density_total'][:, 0, 0] \
